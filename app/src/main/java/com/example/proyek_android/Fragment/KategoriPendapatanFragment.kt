...
import com.example.proyek_android.Classes.KategoriPemasukkan
...
class KategoriPendapatanFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_kategori_pendapatan, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val lv_kategori_pendapatan = view.findViewById<ListView>(R.id.lv_kategori_pendapatan)

        val listKategoriPemasukkan = ArrayList<String>()

        for (kategori in user.kategoriPemasukkan){
            listKategoriPemasukkan.add(kategori.icon + " " + kategori.nama)
        }

        val adapter = ArrayAdapter(view.context, android.R.layout.simple_list_item_1, listKategoriPemasukkan)
        lv_kategori_pendapatan.adapter = adapter

        lv_kategori_pendapatan.setOnItemClickListener { adapterView, view, i, l ->
            //add kategori dialog
            ...
            btn_tambah_kategori.setOnClickListener {
                ...
                if (radio_grup.checkedRadioButtonId == -1){
                    ...
                    if (user.kategoriPemasukkan[i].nama != nama_kategori){
                ...
                if (nama_kategori.isNotBlank() && radio_grup.checkedRadioButtonId != -1 && !kembar){
                    ...
                    user.editKategoriPemasukan(i, nama_kategori, icon)
                    load_list_adapter(user.kategoriPemasukkan, listKategoriPemasukkan)
    ...
    public fun load_list_adapter(allKategori : ArrayList<KategoriPemasukkan>, listKategoriPemasukan : ArrayList<String>){
        listKategoriPemasukan.clear()
        for (kategori in allKategori){
            listKategoriPemasukan.add(kategori.icon + " " + kategori.nama)
        }
    }
}